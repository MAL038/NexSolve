...
interface SidebarProps {
  ...
  isSuperuser?: boolean;
  isOrgAdmin?: boolean; // of huidige user org-admin is (owner/admin)
  orgId?: string | null; // (nu niet gebruikt)
  orgName?: string | null; // (nu niet gebruikt)
  onNavigate?: () => void;
}
...
export default function Sidebar({
  ...
  const visibleCalendarItems = CALENDAR_ITEMS.filter((item) => item.roles.includes(userRole));
    ...
    <aside className="flex flex-col w-64 min-h-screen bg-white border-r border-slate-100 py-6 px-4 flex-shrink-0">
      ...
      <nav className="flex-1 flex flex-col gap-0.5 overflow-y-auto">
        ...
        <div className="space-y-0.5">
          <NavItem href="/projects" icon={FolderKanban} label="Projecten" active={isProjectsArea} onNavigate={onNavigate} />
          {/* (hierarchy wordt nu niet gerenderd in jouw snippet; laat ik intact) */}
        </div>
        ...
        {/* Urenregistratie */}
        <NavItem href="/hours" icon={Clock} label="Urenregistratie" active={pathname.startsWith("/hours")} onNavigate={onNavigate} />
        ...
        <div className="space-y-0.5">
          ...
          <button
            ...
            className={clsx(
              ...
              isCalendarArea ? "bg-brand-500 text-white shadow-sm shadow-brand-200" : "text-slate-600 hover:bg-slate-50 hover:text-brand-600"
            )}
          >
            <Calendar size={18} />
            <span className="flex-1 text-left">Kalender</span>
            <ChevronDown size={14} className={clsx("transition-transform duration-200", calendarExpanded && "rotate-180")} />
          </button>
                ...
                return (
                  <Link
                    ...
                    className={clsx(
                      ...
                      active ? "bg-brand-50 text-brand-700 font-semibold" : "text-slate-500 hover:bg-slate-50 hover:text-brand-600"
                    )}
                  >
                    <item.icon size={13} />
                    {item.label}
        ...
        {/* Org-admin (owner/admin) ziet Beheer voor eigen org */}
        {isOrgAdmin && !isSuperuser && (
          <NavItem
            href="/beheer"
            icon={Landmark}
            label="Beheer"
            active={pathname.startsWith("/beheer")}
            onNavigate={onNavigate}
          />
        )}

        {/* Superuser ziet Organisaties */}
        {isSuperuser && (
          <NavItem
            href="/organisaties"
            icon={Crown}
            label="Organisaties"
            active={pathname.startsWith("/organisaties")}
            onNavigate={onNavigate}
          />
        )}
