...
export function DossierCreateModal({ projectId, customerId, onCreated, onCancel }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Stap 1: upload bestand als aanwezig
      if (file) {
        const formData = new FormData()
        formData.append('file', file)
        
        const uploadRes = await fetch('/api/dossiers/upload', {
          method: 'POST',
          body: formData,
        })
          ...
          <div>
            ...
            {file ? (
              ...
              <button
                ...
                className="w-full px-3 py-2 border-2 border-dashed border-gray-300 rounded-lg text-sm text-gray-500 hover:border-blue-400 hover:text-blue-500"
              >
